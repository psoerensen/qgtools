...
if (error) {
  throw std::runtime_error(error_msg ? error_msg : "Unknown BED read error.");
}
}

extern "C" void mtgrsbed_core_c(
    const char* file,
    int n,
    const int* cls,
    const double* af,
  ...
  try {
    ...
    // For a real production shared library, you may want an explicit
    // error callback or status code. For concept illustration, swallow.
  }
}
